...
export default function Home() {
  ...
  useEffect(() => {
    ...
    fetch(
      "https://api.openweathermap.org/data/2.5/weather?q=SanDiego,us&appid=" +
        apiKey +
        "&units=imperial"
    )
      .then((response) => {
        if (!response.ok) {
          throw new Error(`Status Code: ${response.status}`); //if the response is not ok, throw an error
        }
    ...
    <div>
      ...
      <form id="search-form">
        <input
          type="text"
          id="search-input"
          className="form-input w-100 fs-3"
          placeholder="Enter a city name"
        />
        <button
          type="submit"
          id="submit button"
          className="col-12 bg-info fs-4 my-4 p-2 border-dark"
        >
          Submit
        </button>
      </form>
